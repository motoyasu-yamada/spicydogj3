...
void CMailMessage::prepare_header()
{
   ...
   //*** End
	...
	add_header_line(GetCurrentRFC822Date());
...
CString GetCurrentRFC822Date() {
	...
	format.Format(
		"Date: %s, %02d %s %d %02d:%02d:%02d +%02d00",
		(char*)szDays[current->tm_wday],
		current->tm_mday,
		(char*)szMonths[current->tm_mon],
